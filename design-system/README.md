...
- `assets/` — cópias locais do logo InovvaCar e imagens usadas como referência visual na demonstração.
- `assets/brand/inovvacar-logo-claro.png` — logo claro, com lettering branco, para fundos `Deep Ink` e superfícies escuras.
- `assets/brand/inovvacar-logo-escuro.png` — logo escuro, com lettering `Deep Ink`, para fundos claros.
- As duas versões do logo são PNG RGBA com transparência real; o xadrez de visualização não faz parte dos arquivos.
...
- Hero Institucional: composição de filme do Sonic Link — palco central, wave, títulos em fases, elemento central, cards flutuantes, flash e meta rail — traduzida para a paleta e tipografia InovvaCar.
...
- **Thor** — hero frame a frame e cards modernos; reservado às páginas internas do Showroom, com adaptação da temática para a linguagem InovvaCar.

As imagens em `assets/references/` são amostras visuais copiadas localmente para a documentação. Elas não são assets finais de campanha nem de veículos. Os componentes incluem slots substituíveis para receber fotografia, vídeo ou sequência aprovada da operação.
